...
class Leakage(Resource):
    def get(self):
        ...
        parser.add_argument('t_count', type=str, help='')
        ...
        if args.get('t_count'):
            filters = dict({'t_count': int(args.get('t_count'))}, **filters)
        results = list(
            result_col.find(filters, {'code': 0, }).sort('datetime', DESCENDING).limit(
                args.get('limit')).skip(
