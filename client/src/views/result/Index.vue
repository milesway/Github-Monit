        ...
        <el-card shadow="never">
            <el-form class="filter-panel">
                <el-row :type="mobileClient?'':'flex'">
                    ...
                    <el-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
                        <el-form-item label="威胁数量">
                            <el-select v-model="filters.t_count"
                                       size="small"
                                       filterable
                                       clearable
                                       placeholder="请选择威胁数量"
                                       @change="handleFilter"
                                       @focus="fetchStatisticsData('t_count')"
                            >
                                <el-option
                                        v-for="item in statistics.t_count"
                                        :key="item._id"
                                        :value="item._id">
                                    {{item._id}} <span class="tag-count">{{item.value}}</span>
                                </el-option>
...
<script>
    ...
    export default {
        data() {
            return {
                statistics: {tag: [], language: [], t_count:[]},
                filters: {
                    status: this.$route.query.status || "待审",
                    tag: this.$route.query.tag || "",
                    language: this.$route.query.language || "",
                    t_count: this.$route.query.t_count || ""
                },
                trendData: {},
                leakagesData: [],
                total: 10
        ...
        computed: {
            ...
            ,
            t_count() {
                if (this.$route.query.t_count) {
                    return this.$route.query.t_count;
                } else {
                    return null;
                }
            }
        ...
        methods: {
            handleStatusFilter(status) {
                this.$router.push({
                    name: "index",
                    query: {page: 1, limit: 10, tag: this.filters.tag, language: this.filters.language, t_count: this.filters.t_count, status: status}
                });
            }
            ,
            handleFilter() {
            ...
            handleSizeChange(val) {
                this.$router.push({
                    name: "index",
                    query: {page: 1, limit: val, tag: this.tag, language: this.language, t_count: this.t_count, status: this.filters.status}
                });
            }
            ,
